...
/** Collapse a version chain to the highest version per family. */
function collapseFamilies<T extends { family_id: string; version: number | string }>(
  rows: T[],
): T[] {
  const byFamily = new Map<string, T>();
  for (const r of rows) {
    const current = byFamily.get(r.family_id);
    if (!current || Number(r.version) > Number(current.version)) {
      byFamily.set(r.family_id, r);
    }
  }
...
export async function getLibraryStaves(
  ...
  const result = await db.execute(sql`
    SELECT s.id, s.family_id, s.slug, s.title, s.tags, s.version, s.status,
           s.updated_at,
    ...
    LIMIT ${LIMIT}
  `);
  const rows = collapseFamilies(result as unknown as RawStaveRow[]);
  return rows.map((r) => ({
    id: r.id,
    slug: r.slug,
    title: r.title,
...
export async function getLibraryGrimoires(
  ...
  const result = await db.execute(sql`
    SELECT g.id, g.family_id, g.slug, g.title, g.tags, g.version, g.status,
    ...
    LIMIT ${LIMIT}
  `);
  const rows = collapseFamilies(result as unknown as RawGrimoireRow[]);
  return rows.map((r) => ({
    id: r.id,
    slug: r.slug,
    title: r.title,
